...
// OTOİZ Aşama 1 — kalıcı QR adresi (go.<final-domain>/<token>) ve resolver'ın
// yönlendirdiği ana uygulama adresi. Fiziksel QR basıldıktan sonra bu adres
// değiştirilemeyeceği için Production build'i YANLIŞ değerle başlamaz.
//   - production: NEXT_PUBLIC_QR_BASE_URL tanımlıysa https, "go." ile
//     başlayan host, path/sorgu yok, *.vercel.app değil; bu durumda
//     OTOIZ_APP_ORIGIN zorunlu (https, path yok, QR host'undan farklı,
//     *.vercel.app değil). Tanımlı değilse build geçer, QR üretimi kilitli.
//   - staging: ikisi de isteğe bağlı; tanımlıysa https olmalı. QR adresi
//     staging'de path içerebilir (ör. https://<staging>.vercel.app/r).
function validateQrUrls(env, role) {
  const errors = [];
  const qrRaw = String(env.NEXT_PUBLIC_QR_BASE_URL || "").trim();
  const appRaw = String(env.OTOIZ_APP_ORIGIN || "").trim();
  const isProd = role === "production";

  // Tanımlı değilse build/deploy ENGELLENMEZ (QR ile ilgisiz acil düzeltmeler
  // çıkabilsin); bunun yerine QR üretimi ve basımı uygulama içinde kilitlidir
  // (lib/qrUrl.js qrIssuanceLocked). Tanımlıysa değer sıkı doğrulanır.
  if (!qrRaw) return errors;

  let qr = null;
  try {
    qr = new URL(qrRaw);
  } catch {
    errors.push("NEXT_PUBLIC_QR_BASE_URL geçerli bir URL değil.");
  }
  if (qr) {
    if (qr.protocol !== "https:") errors.push("NEXT_PUBLIC_QR_BASE_URL https:// kullanmalı.");
    if (qr.search || qr.hash) errors.push("NEXT_PUBLIC_QR_BASE_URL sorgu (?) veya # içeremez.");
    if (isProd) {
      if (!qr.hostname.startsWith("go.")) errors.push(`NEXT_PUBLIC_QR_BASE_URL hostu "go." ile başlamalı (bulunan: "${qr.hostname}").`);
      if (qr.pathname !== "/") errors.push("NEXT_PUBLIC_QR_BASE_URL Production'da path içeremez (yalnız https://go.<alan-adı>).");
      if (qr.hostname.endsWith(".vercel.app")) errors.push("NEXT_PUBLIC_QR_BASE_URL Production'da *.vercel.app olamaz.");
    }
  }

  if (!appRaw) {
    errors.push("OTOIZ_APP_ORIGIN tanımlı değil — NEXT_PUBLIC_QR_BASE_URL tanımlıysa resolver'ın yönlendireceği ana adres de zorunlu.");
    return errors;
  }
  let app = null;
  try {
    ...
    errors.push("OTOIZ_APP_ORIGIN geçerli bir URL değil.");
  }
  if (app) {
    if (app.protocol !== "https:") errors.push("OTOIZ_APP_ORIGIN https:// kullanmalı.");
    if (app.pathname !== "/" || app.search || app.hash) errors.push("OTOIZ_APP_ORIGIN yalnız origin olmalı (path/sorgu yok).");
    if (qr && isProd && app.hostname === qr.hostname) errors.push("OTOIZ_APP_ORIGIN, QR hostu ile aynı olamaz.");
    if (isProd && app.hostname.endsWith(".vercel.app")) errors.push("OTOIZ_APP_ORIGIN Production'da *.vercel.app olamaz.");
  }
  return errors;
}
...
function validateDeploymentEnv(env) {
  ...
  errors.push(...validateQrUrls(env, role));
  if (!String(env.NEXT_PUBLIC_QR_BASE_URL || "").trim()) {
    notices.push("NEXT_PUBLIC_QR_BASE_URL tanımlı değil — deploy devam eder; QR üretimi ve basımı uygulama içinde KİLİTLİ.");
  }
