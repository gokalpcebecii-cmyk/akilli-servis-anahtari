...
// değiştirilemeyeceği için Production build'i yanlış/eksik değerle BAŞLAMAZ.
//   - production: NEXT_PUBLIC_QR_BASE_URL zorunlu; https, "go." ile başlayan
//     host, path/sorgu yok, *.vercel.app değil. OTOIZ_APP_ORIGIN zorunlu;
//     https, path yok, QR host'undan farklı, *.vercel.app değil.
...
function validateQrUrls(env, role) {
  ...
  if (!qrRaw) {
    if (isProd) errors.push("NEXT_PUBLIC_QR_BASE_URL tanımlı değil — Production'da kalıcı QR adresi (https://go.<alan-adı>) zorunlu.");
    return errors;
  }
