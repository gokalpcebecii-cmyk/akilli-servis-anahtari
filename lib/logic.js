...
// Pilot bugfix turu: tüm "bugün" hesapları Europe/Istanbul takvim gününü
// kullanır — `new Date().toISOString()` HER ZAMAN UTC'dir ve gece yarısına
// yakın saatlerde (İstanbul UTC+3) yanlış günü verebilir (ör. İstanbul'da
// 00:30 iken UTC hâlâ bir önceki gün). Intl.DateTimeFormat ile doğrudan
// hedef saat diliminin takvim günü okunur — yerel çalışma zamanı saat
...
function computeMaintenancePlan({ currentKm, planType, customNextKm, customNextDate, today }) {
  ...
  const todayIso = today || todayIsoIstanbul();
  const [ty, tm, td] = todayIso.split("-").map(Number);

  // Ay ekleme, bir Date nesnesinin yerel getter/setter'larına (setMonth +
  // toISOString) DEĞİL, doğrudan takvim bileşenlerine (yıl/ay/gün) göre
  // yapılır — bu, çalışma zamanının (tarayıcı/sunucu) yerel saat dilimi
  // UTC'nin GERİSİNDEYSE `setMonth`+`toISOString` çiftinin günü bir gün
  // GERİYE kaydırma riskini ortadan kaldırır (yalnızca ay/gün aritmetiği,
  // saat dilimi dönüşümü YOK).
  function addMonthsIso(months) {
    const totalMonths = ty * 12 + (tm - 1) + months;
    const y2 = Math.floor(totalMonths / 12);
    const m2 = totalMonths % 12; // 0-11
    const daysInMonth = new Date(Date.UTC(y2, m2 + 1, 0)).getUTCDate();
    const d2 = Math.min(td, daysInMonth);
...
// İkinci düzeltme turu (madde 8): özel plan girişindeki sonraki bakım
// TARİHİ de geçmişte olamaz (UI'de yalnızca <input min> ile engelleniyordu
// — sunucu tarafında hiç doğrulanmıyordu; bu, /api/vehicles'ı UI'yi
// atlayan doğrudan bir istekle geçmiş tarihli plan yazılabilir hale
// getiriyordu). todayISO verilmezse gerçek bugünün tarihi kullanılır.
function isValidNextServiceDate(nextServiceDate, todayISO) {
  if (!nextServiceDate) return true;
  const today = todayISO || todayIsoIstanbul();
  return nextServiceDate >= today;
}

// Servis hızlı bakım kaydı: yeni girilen güncel km, kayıtlı ESKİ km'den
// düşük olamaz — pilot bugfix turu, next_service_km/tarih ezilme
// hatasıyla birlikte bulunan ilgili bir düzeltme.
function isValidCurrentKmUpdate(previousKm, newKm) {
  if (previousKm == null) return true;
  if (newKm === "" || newKm == null || Number.isNaN(Number(newKm))) return false;
  return Number(newKm) >= Number(previousKm);
}

// Servis hızlı bakım kaydı — otomatik "sonraki bakım" önerisi: varsayılan
// taban +10.000 km / +12 ay'dır; seçili bakım kalemlerinden HERHANGİ biri
// bu tabandan DAHA ERKEN (daha küçük mutlak km) bir vadeye işaret ediyorsa
// o kazanır. Manuel "Planı düzenle" girişi çağıran tarafından (yalnızca
// kullanıcı gerçekten doldurduysa) BU sonucun üzerine yazılır — bu
// fonksiyon manuel geçersiz kılmayı BİLMEZ, yalnızca otomatik öneriyi
// hesaplar (tek sorumluluk, çağrı yerinde birleştirilir).
function computeAutoNextServicePlan({ currentKm, today, selectedItemIntervals }) {
  const km = Number(currentKm) || 0;
  const basePlan = computeMaintenancePlan({ currentKm: km, planType: "default", today });
  let nextServiceKm = basePlan.nextServiceKm;
  for (const interval of Object.values(selectedItemIntervals || {})) {
    const intervalNum = interval === "" || interval == null ? null : Number(interval);
    if (intervalNum) {
      const candidate = km + intervalNum;
      if (candidate < nextServiceKm) nextServiceKm = candidate;
    }
  }
  return { nextServiceKm, nextServiceDate: basePlan.nextServiceDate };
}
